   ...
   if(command.ReadMsgs() > 0)
   {
    if(command.id == ID || command.id ==SYNC_ID)
    {
        if(command.lookV < (-1 * DEADBAND))
        {
           ...
           leftServo.write(map(command.lookV, 0, -127, 90, PWM_MIN));                  // sets the servo position according to the scaled value 
        ...
        else if (command.lookV > DEADBAND)
        {
          ...
          leftServo.write(map(command.lookV, 0, 127, 90, PWM_MAX));                  // sets the servo position according to the scaled value 
 


          tempSpeed = -1*map(command.lookV, 0, 127, 90, PWM_MAX);
